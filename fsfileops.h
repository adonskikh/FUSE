...
long Remove(struct dinode *dir, long index, struct dinode *parent)
{
    ...
    if(dir->di_mode | S_IFDIR)
    {
        ...
        for(i = 0; i < (dir->di_size)/sizeof(struct dirent); i++)
        {
            if(!(strcmp(items[i].d_name, "..")==0) && !(strcmp(items[i].d_name, ".")==0))
            {
                ...
                if(child.di_size < 0)
                    return -EIO;
                if(Remove(&child, items[i].d_ino, dir)<0)
                    return -EIO;
            }
        }
    }
...
long Rename(const char *path, const char *newpath)
{
    ...
    printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!1 path = %s, index = %ld, size = %ld\n", newpath, new_parent_index, new_parent->di_size);
    ...
    AddHardLink(new_parent, &n, index, name);
    (new_parent->di_nlink)++;
